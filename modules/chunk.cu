...
int Chunk::enqueue(int batchSize, const void* const* inputs,
    ...
    for (int b = 0; b < batchSize; ++b) {
        ...
        /*
                NV_CUDA_CHECK(cudaMemcpyAsync((char*)outputs[0] + b *
           _n_size_split, (char*)inputs[0] + b * 2 * _n_size_split +
           _n_size_split/sizeof(float), _n_size_split, cudaMemcpyDeviceToDevice,
           stream)); NV_CUDA_CHECK(cudaMemcpyAsync((char*)outputs[1] + b *
           _n_size_split, (char*)inputs[0] + b * 2 * _n_size_split +
           _n_size_split/sizeof(float), _n_size_split, cudaMemcpyDeviceToDevice,
           stream));
                */
