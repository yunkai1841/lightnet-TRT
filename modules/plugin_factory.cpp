...
YoloLayer::YoloLayer(const uint32_t& numBoxes, const uint32_t& numClasses,
    ...
    assert(_n_grid_w > 0);
    m_OutputSize =
        _n_grid_h * _n_grid_w * (m_NumBoxes * (4 + 1 + m_NumClasses));
}
...
bool YoloV4Layer::supportsFormat(DataType type,
                                 PluginFormat format) const noexcept {
    return (type == DataType::kFLOAT && format == PluginFormat::kLINEAR);
}

void YoloV4Layer::configureWithFormat(const Dims* inputDims, int nbInputs,
                                      const Dims* outputDims, int nbOutputs,
                                      DataType type, PluginFormat format,
                                      int maxBatchSize) noexcept {}

IPluginV2* YoloV4Layer::clone() const noexcept {
    YoloV4Layer* p = new YoloV4Layer(m_NumBoxes, m_NumClasses, _n_grid_h,
                                     _n_grid_w, m_new_coords, m_scale_x_y);
    p->setPluginNamespace(_s_plugin_namespace.c_str());
    return p;
}

YoloV4Layer::YoloV4Layer(const uint32_t& numBoxes, const uint32_t& numClasses,
                         const uint32_t& grid_h_, const uint32_t& grid_w_,
...
nvinfer1::Dims YoloV4Layer::getOutputDimensions(int index,
                                                const nvinfer1::Dims* inputs,
                                                int nbInputDims) noexcept {
    assert(index == 0);
    assert(nbInputDims == 1);
...
int YoloV4Layer::enqueue(int batchSize, const void* const* inputs,
                         void* const* outputs, void* workspace,
                         cudaStream_t stream) noexcept {
    NV_CUDA_CHECK(cudaYoloV4Layer(
        inputs[0], outputs[0], batchSize, _n_grid_h, _n_grid_w, m_NumClasses,
...
size_t YoloV4Layer::getSerializationSize() const noexcept {
    printf("Serialize V4\n");
    return sizeof(m_NumBoxes) + sizeof(m_NumClasses) + sizeof(_n_grid_w) +
           sizeof(_n_grid_h) + sizeof(m_OutputSize) + sizeof(m_scale_x_y) +
           sizeof(m_new_coords);
}
...
YoloV4LayerPluginCreator::getFieldNames() noexcept {
    return &mFC;
}
...
void YoloV4LayerPluginCreator::setPluginNamespace(
    const char* libNamespace) noexcept {
    mNamespace = libNamespace;
}
