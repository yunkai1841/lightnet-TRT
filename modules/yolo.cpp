...
Yolo::Yolo(const NetworkInfo& networkInfo, const InferParams& inferParams)
    ...
    if (networkInfo.width && networkInfo.height) {
        ...
        m_EnginePath = networkInfo.data_path + "-" + std::to_string(m_InputW) +
                       "x" + std::to_string(m_InputH) + "-" + m_Precision;
    ...
    if (m_Precision == "kFLOAT") {
        ...
        Int8EntropyCalibrator calibrator(
            m_BatchSize, m_CalibImages, m_CalibImagesFilePath,
    ...
    m_Engine =
        loadTRTEngine(m_EnginePath, /* m_PluginFactory,*/ m_Logger, m_dla);
...
void Yolo::createYOLOEngine(const nvinfer1::DataType dataType,
    ...
    nvinfer1::ITensor* data = m_Network->addInput(
        ...
        nvinfer1::Dims{4, static_cast<int>(1), static_cast<int>(m_InputC),
    ...
    for (uint32_t i = 0; i < m_configBlocks.size(); ++i) {
        ...
        if (m_configBlocks.at(i).at("type") == "net") {
            ...
            if (m_configBlocks.at(i).find("sparse") !=
            ...
            if ((m_configBlocks.at(i).find("batch_normalize") !=
                ...
                layerType = "conv-bn-relu";
                        ...
                        m_configBlocks.at(i).end()) &&
                       ("swish" == activation)) {
                out = netAddConvBNSwish(i, m_configBlocks.at(i), weights,
                                        trtWeights, weightPtr, channels,
                                        previous, m_Network);
                layerType = "conv-bn-swish";
                        ...
                        m_configBlocks.at(i).end()) &&
                       ("mish" == activation)) {
                out = net_conv_bn_mish(i, m_configBlocks.at(i), weights,
                                       trtWeights, weightPtr, channels,
                                       previous, m_Network);
                ...
                if ("linear" == activation) {
                    ...
                    out = netAddConvSigmoid(i, m_configBlocks.at(i), weights,
                                            trtWeights, weightPtr, channels,
                                            previous, m_Network);
            ...
            if (m_configBlocks.at(i).find("scale_x_y") !=
                m_configBlocks.at(i).end()) {
            ...
            if (m_configBlocks.at(i).find("colormap") !=
                m_configBlocks.at(i).end()) {
                std::string colormapString =
                    m_configBlocks.at(i).at("colormap");
                while (!colormapString.empty()) {
                    size_t npos = colormapString.find_first_of(',');
                    if (npos != std::string::npos) {
                        uint32_t colormap = (uint32_t)std::stoi(
                            trim(colormapString.substr(0, npos)));
                        curYoloTensor.colormap.push_back(colormap);
                        colormapString.erase(0, npos + 1);
                    } else {
                        int colormap =
                            (uint32_t)std::stoi(trim(colormapString));
                        curYoloTensor.colormap.push_back(colormap);
                        break;
                    }
                }
            ...
            if (m_configBlocks.at(i).find("names") !=
                m_configBlocks.at(i).end()) {
            ...
            std::string softmaxLayerName =
                "segmenter_" + std::to_string(segmenter_count);
            ...
            curYoloTensor.volume = curYoloTensor.grid_h * curYoloTensor.grid_w *
            ...
            if (m_configBlocks.at(i).find("colormap") !=
                ...
                while (!colormapString.empty()) {
                    ...
                    if (npos != std::string::npos) {
                        uint32_t colormap =
                            std::stoi(trim(colormapString.substr(0, npos)));
                        curYoloTensor.colormap.push_back(colormap);
                        colormapString.erase(0, npos + 1);
                    } else {
            ...
            if (m_configBlocks.at(i).find("depth") !=
                m_configBlocks.at(i).end()) {
            ...
            if (found != std::string::npos)  // concate multi layers
            {
                ...
                for (auto& ind_layer : vec_index) {
                    if (ind_layer < 0) {
                        ind_layer =
                            static_cast<int>(tensorOutputs.size()) + ind_layer;
                    }
            ...
            } else  // single layer
            {
                ...
                assert(idx < static_cast<int>(tensorOutputs.size()) &&
                       idx >= 0);

                // route
                if (m_configBlocks.at(i).find("groups") ==
                    m_configBlocks.at(i).end()) {
                    previous = tensorOutputs[idx];
                    assert(previous != nullptr);
                    std::string outputVol =
                        dimsToString(previous->getDimensions());
                ...
                // yolov4-tiny route split layer
                    ...
                    auto start = nvinfer1::Dims{dim.nbDims,
                                                {0, dim.d[1] / groups, 0, 0}};
                    // auto start = nvinfer1::Dims{dim.nbDims, {0, 0, 0}};
                    auto size = nvinfer1::Dims{
                        dim.nbDims, {1, dim.d[1] / groups, dim.d[2], dim.d[3]}};
                    auto stride = nvinfer1::Dims{4, 1, 1, 1, 1};
                    nvinfer1::ISliceLayer* out = m_Network->addSlice(
                        *(tensorOutputs[idx]), start, size, stride);
                    std::string inputVol =
                        dimsToString(previous->getDimensions());
                    std::string sliceLayerName =
                        "slice_" + std::to_string(i - 1);
                    ...
                    std::string outputVol =
                        dimsToString(previous->getDimensions());
                    tensorOutputs.push_back(out->getOutput(0));
                    printLayerInfo(layerIndex, "slice", inputVol, outputVol,
                                   std::to_string(weightPtr));
                }
            }
            ...
            nvinfer1::Dims d =
                nvinfer1::Dims{4, 1, previous->getDimensions().d[1],
                               previous->getDimensions().d[2] * 2,
    ...
    std::cout << "Total GFLOPS " << std::to_string(total_gflops)
              << " Total MACC " << std::to_string(total_gflops / 2)
              << std::endl;
    std::cout << "Total Params[M] "
              << std::to_string(total_num_params / (float)(1000 * 1000))
              << std::endl;
    ...
    if (m_dla != -1) {
        ...
        if (ndla > 0) {
            std::cout << "###" << ndla << " DLAs are supported! ###"
                      << std::endl;
        } else {
            std::cout << "###Warninig : "
    ...
    if (dataType == nvinfer1::DataType::kINT8) {
        assert((calibrator != nullptr) &&
               "Invalid calibrator for INT8 precision");
...
void Yolo::preprocess_gpu(unsigned char* src, int w, int h) {
    ...
    if (!m_h_img) {
        NV_CUDA_CHECK(
            cudaMallocHost((void**)&m_h_img, sizeof(unsigned char) * src_size));
        NV_CUDA_CHECK(
            cudaMalloc((void**)&m_d_img, sizeof(unsigned char) * src_size));
    }
    // Copy into pinned memory
    memcpy(m_h_img, src, src_size * sizeof(unsigned char));
    // Copy into device memory
    NV_CUDA_CHECK(cudaMemcpyAsync(m_d_img, m_h_img,
                                  src_size * sizeof(unsigned char),
                                  cudaMemcpyHostToDevice, m_CudaStream));

    blobFromImageGpu((float*)(m_DeviceBuffers.at(m_InputBindingIndex)), m_d_img,
                     m_InputW, m_InputH, m_InputC, w, h, 3, norm, m_CudaStream);
}
...
void Yolo::doInference(const unsigned char* input, const uint32_t batchSize) {
    ...
    if (input) {
        NV_CUDA_CHECK(cudaMemcpyAsync(m_DeviceBuffers.at(m_InputBindingIndex),
                                      input,
                                      batchSize * m_InputSize * sizeof(float),
                                      cudaMemcpyHostToDevice, m_CudaStream));
    }

    // m_Context->enqueue(batchSize, m_DeviceBuffers.data(), m_CudaStream,
    // nullptr);
...
std::vector<cv::Mat> Yolo::apply_argmax(const int& imageIdx) {
    ...
    for (auto& tensor : m_OutputTensors) {
        if (tensor.segmenter) {
            ...
            cv::Mat argmax =
                cv::Mat::zeros(tensor.grid_h, tensor.grid_w, CV_8UC1);
            ...
            for (int y = 0; y < tensor.grid_h; y++) {
                for (int x = 0; x < tensor.grid_w; x++) {
                    ...
                    for (int c = cstart; c < tensor.numClasses; c++) {
                        float value = prob[c * tensor.grid_h * tensor.grid_w +
                                           y * tensor.grid_w + x];
                        if (max < value) {
                            max = value;
                            index = c;
...
std::vector<cv::Mat> Yolo::get_colorlbl(std::vector<cv::Mat>& argmax) {
    ...
    for (auto& tensor : m_OutputTensors) {
        if (tensor.segmenter && !tensor.depth) {
            ...
            cv::Mat mask =
                cv::Mat::zeros(tensor.grid_h, tensor.grid_w, CV_8UC3);
            for (int y = 0; y < tensor.grid_h; y++) {
                for (int x = 0; x < tensor.grid_w; x++) {
...
std::vector<cv::Mat> Yolo::get_depthmap(std::vector<cv::Mat>& argmax) {
    ...
    for (auto& tensor : m_OutputTensors) {
        if (tensor.segmenter && tensor.depth) {
            cv::Mat gray = argmax[count];
            /*
            cv::Mat mask = cv::Mat::zeros(tensor.grid_h, tensor.grid_w,
            CV_8UC3);      int c = tensor.numClasses; std::cout << c <<
            std::endl; for (int y = 0; y < tensor.grid_h; y++) { for (int x = 0;
            x < tensor.grid_w; x++) { int id = gray.at<unsigned char>(y, x);
                mask.at<cv::Vec3b>(y, x)[0] = (unsigned char)(255 *
            id/(float)c); mask.at<cv::Vec3b>(y, x)[1] = (unsigned char)(255 *
            id/(float)c); mask.at<cv::Vec3b>(y, x)[2] = (unsigned char)(255 *
            id/(float)c);
              }
            }
            */
...
void Yolo::parseConfigBlocks() {
    ...
    for (auto block : m_configBlocks) {
        if (block.at("type") == "net") {
            ...
            m_InputSize = m_InputC * m_InputH * m_InputW;
        } else if ((block.at("type") == "region") ||
                   (block.at("type") == "yolo")) {
            assert((block.find("num") != block.end()) &&
                   std::string("Missing 'num' param in " + block.at("type") +
                               " layer")
                       .c_str());
            assert((block.find("classes") != block.end()) &&
                   std::string("Missing 'classes' param in " +
                               block.at("type") + " layer")
                       .c_str());
            assert((block.find("anchors") != block.end()) &&
                   std::string("Missing 'anchors' param in " +
                               block.at("type") + " layer")
                       .c_str());
            ...
            while (!anchorString.empty()) {
                ...
                if (npos != std::string::npos) {
                    float anchor =
                        std::stof(trim(anchorString.substr(0, npos)));
            ...
            if ((m_NetworkType == "yolov3") ||
                ...
                assert((block.find("mask") != block.end()) &&
                       std::string("Missing 'mask' param in " +
                                   block.at("type") + " layer")
                           .c_str());
                ...
                while (!maskString.empty()) {
                    ...
                    if (npos != std::string::npos) {
                        uint32_t mask =
                            std::stoul(trim(maskString.substr(0, npos)));
            ...
            if (block.find("new_coords") != block.end()) {
                outputTensor.blobName =
                    "ScaledYoloV4_" + std::to_string(_n_yolo_ind);
            } else {
            ...
            if (m_NetworkType == "yolov4") {
                outputTensor.gridSize =
                    (m_InputH / 32) * pow(2, 2 - _n_yolo_ind);
...
void Yolo::allocateBuffers() {
    ...
    for (auto& tensor : m_OutputTensors) {
        tensor.bindingIndex =
            m_Engine->getBindingIndex(tensor.blobName.c_str());
        assert((tensor.bindingIndex != -1) && "Invalid output binding index");
        if (tensor.segmenter) {
            NV_CUDA_CHECK(
                cudaMalloc(&m_DeviceBuffers.at(tensor.bindingIndex),
                           m_BatchSize * tensor.volume * sizeof(float)));
            NV_CUDA_CHECK(
                cudaMallocHost(&tensor.hostBuffer,
...
bool Yolo::verifyYoloEngine() {
    ...
    for (auto tensor : m_OutputTensors) {
        ...
        assert(get4DTensorVolume(m_Engine->getBindingDimensions(
