...
class YoloDectector {
   public:
    ...
    void save_image(cv::Mat &img, const std::string &dir,
    ...
    void draw_BBox(cv::Mat &img, Result result) {
        ...
        int id = result.id;
        if (colormap) {
            cv::rectangle(img, result.rect,
                          cv::Scalar(colormap[3 * id + 2], colormap[3 * id + 1],
                                     colormap[3 * id + 0]),
                          4);
        } else {
        ...
        if (names.size()) {
            stream << std::fixed << std::setprecision(2) << names[id] << "  "
                   << result.prob;
        } else {
            stream << std::fixed << std::setprecision(2) << "id:" << id
                   << "  score:" << result.prob;
        }

        if (colormap) {
            cv::putText(img, stream.str(),
                        cv::Point(result.rect.x, result.rect.y - 5), 0, 0.5,
                        cv::Scalar(colormap[3 * id + 2], colormap[3 * id + 1],
                                   colormap[3 * id + 0]),
                        1);
        } else {
    ...
    void segment(const std::vector<cv::Mat> &vec_image, std::string filename) {
        ...
        for (uint32_t i = 0; i < vec_image.size(); ++i) {
            ...
            for (uint32_t j = 0; j < mask.size(); j++) {
                ...
                cv::resize(mask[j], resized, cv::Size(width, height), 0, 0,
                           cv::INTER_NEAREST);
                cv::addWeighted(vec_image[i], 1.0, resized, 0.5, 0.0,
                                vec_image[i]);
                ...
                if (flg_save) {
                    ...
                    if (0) {
                        ...
                        sout << std::setfill('0') << std::setw(6)
                             << GLOBAL_COUNTER;
                        // cv::resize(mask[j], resized, cv::Size(width, height),
    ...
    void detect(const std::vector<cv::Mat> &vec_image,
        ...
        if (cuda) {
            ...
            _p_net->preprocess_gpu(vec_image[0].data, vec_image[0].cols,
                                   vec_image[0].rows);
            trtInput.data = NULL;
        } else {
            //      std::cout << "Preprocess on CPU" << std::endl;
            trtInput = blobFromDsImages(vec_ds_images, _p_net->getInputH(),
                                        _p_net->getInputW());
        }
        if (get_prof_flg()) {
        ...
        for (uint32_t i = 0; i < vec_ds_images.size(); ++i) {
            ...
            //	auto segmentation = _p_net->apply_argmax(i,
            //curImage.getImageHeight(), curImage.getImageWidth());
            auto remaining = nmsAllClasses(_p_net->getNMSThresh(), binfo,
   ...
   private:
    ...
    void build_net() {
        if (_config.net_type == YOLOV3) {
            _p_net =
                std::unique_ptr<Yolo>{new YoloV3(_yolo_info, _infer_param)};
        } else if (_config.net_type == YOLOV4) {
            _p_net =
                std::unique_ptr<Yolo>{new YoloV4(_yolo_info, _infer_param)};
        } else if (_config.net_type == YOLOV5) {
            _p_net =
                std::unique_ptr<Yolo>{new YoloV5(_yolo_info, _infer_param)};
        } else {
