...
Detect::Detect(const void* data, size_t length) {
    ...
    // _n_classes, _n_grid_h, _n_grid_w, _n_output_size);
    assert(d == a + length);
}
...
__global__ void gpu_detect_layer(const float* input_, float* output_,
    ...
    output_[bbindex + numGridCells * (z_id * (5 + n_classes_) + 0)] =
        2.f * sigmoidGPU(input_[bbindex +
                                numGridCells * (z_id * (5 + n_classes_) + 0)]) -
        0.5f;

    output_[bbindex + numGridCells * (z_id * (5 + n_classes_) + 1)] =
        2.f * sigmoidGPU(input_[bbindex +
                                numGridCells * (z_id * (5 + n_classes_) + 1)]) -
        0.5f;
...
cudaError_t cuda_detect_layer(const void* input_, void* output_,
    ...
    for (int batch = 0; batch < batch_size_; ++batch) {
        gpu_detect_layer<<<number_of_blocks, threads_per_block, 0, stream_>>>(
            ...
            grid_h_, grid_w_, n_classes_, n_anchor_);
    }
